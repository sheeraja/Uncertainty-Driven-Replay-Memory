...
class Logger:
    def __init__(self, log_folder_details=None, train_details=None, alpha=None):
        self.memory = {}
        ...
        if self.log_folder_details is None:
            directory = folder_dir + 'results/' + game_name + '_' + str(self.alpha) + '/' + today.strftime('%Y-%m-%d-%H%M%S')
        else:
            directory = folder_dir + 'results/' + game_name + '_' + str(self.alpha) + '/' + today.strftime('%Y-%m-%d-%H%M%S') + '-' + self.log_folder_details
        
        os.makedirs(directory)
        self.log_folder = directory
        self.root_folder = folder_dir + 'results/' + game_name + '_' + str(self.alpha) + '/'
        n = len(directory) + 15
        dashes = "-" * n
