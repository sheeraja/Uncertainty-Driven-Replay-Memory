...
from classic_args import Args
from classic_uadqn import UADQN
import wandb
...
from models import MLP_Classic_Control
...
for sd in range(args.n_seeds):
    ...
    if env_id in ["CartPole-v1", "LunarLander-v2"]:
    ...
    weight_scale = args.weight_scale[i]
    noise_scale = args.noise_scale[i]
    num_steps = steps[i]
    agent = UADQN(
                env,
                MLP_Classic_Control,
                ...
                epistemic_factor = unc_lambda_ep,
                aleatoric_factor = unc_lambda_al,
                kappa = args.kappa,
...
root_file = agent.logger.root_folder + total_run_time.start_time.strftime('%Y-%m-%d-%H%M%S_Classic_Control-') + game_name + "_info.txt"
