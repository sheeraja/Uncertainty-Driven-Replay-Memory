...
class CEQRDQN():
    ...
    def learn(self, verbose=False):
        ...
        with open(self.logger.log_folder + '/log_output.txt',"w") as f:
            ...
            for timestep in range(self.train_steps):
                ...
                assert id(self.network) != id(self.target_network), "Networks point to the same object!"
    ...
    def train_step(self, transitions):
        ...
        best_action_idx = torch.mean(target1,dim=2).max(1, True)[1].unsqueeze(2).detach()
        q_value_target = target1.gather(1, best_action_idx.repeat(1,1,self.n_quantiles)) # torch.Size([32, 1, 20])

        # Calculate TD target
        td_target = (rewards.unsqueeze(2).repeat(1,1,self.n_quantiles) + (1 - dones.unsqueeze(2).repeat(1,1,self.n_quantiles)) * self.gamma * q_value_target).squeeze() # torch.Size([32, 20])
        ...
        q_value = out.gather(1, actions.unsqueeze(2).repeat(1,1,self.n_quantiles)).squeeze() # torch.Size([32, 20])
