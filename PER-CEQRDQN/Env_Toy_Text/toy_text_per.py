...
from prioritized_replay_buffer import PrioritizedReplayBuffer_Proportional
...
class PER():
    def __init__(
        ...
        self.env = env
        self.replay_buffer = PrioritizedReplayBuffer_Proportional(self.replay_buffer_size)
    ...
    def encode_state(self, state):
        onehot_obs = torch.zeros(self.env.observation_space.n)
        onehot_obs[state] = 1
        return onehot_obs
    ...
    def learn(self, verbose=False):
        ...
        with open(self.logger.log_folder + '/log_output.txt',"w") as f:
            ...
            for timestep in range(self.train_steps):
                ...
                # Select action
                action, least_uncertain, greedy = self.act(timestep, state.to(self.device).float())
                ...
                state_next = self.encode_state(state_next)
                ...
                if done:
                    ...
                    if verbose:
                        log_statement = textcolor + "Timestep: {}, Score: {}, Time: {} s, Actions: {}, ALU: {} ({}%), AG: {} ({}%)".format(timestep, true_score, round(time.time() - t1, 3),
                                                                                                                                    this_episode_time,
                                                                                                                                    actions_uncertain,
    ...
    def train_step(self, transitions):
        states, actions, rewards, states_next, dones, idxs, weights = transitions
        ...
        weights = weights.to(self.device) # torch.Size([32])
        ...
        q_value_target = torch.clamp(q_value_target, -100.0, 100.0)
        ...
        total_loss = quantile_losses + loss_evidence + loss_gamma_cal # torch.Size([32, 20]); torch.Size([32, 20]) + torch.Size([32, 20]) + torch.Size([20])
    ...
    def act(self, t, state):
        ...
        action, least_uncertain, greedy = self.predict(t, state)        
    ...
    def predict(self, t, state):
        ...
        action_means = torch.nan_to_num(action_means)
        ...
        # cov_matrix = torch.diagflat(global_epistemic)
